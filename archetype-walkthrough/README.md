...
With the credentials discovered from the SMB share (`sql_svc` and password `M3g4c0rp123`), we attempt to connect to the Microsoft SQL Server running on port `1433`. Using the **mssqlclient.py** tool from **Impacke**t, we connect with **Windows** authentication.
